...
<body>
    ...
    <!-- <div class="box1">
        ...
        <div class="sidebar">
            ...
            <a href="../storage/manage_storage.php">Manage Inventory</a>
            <a href="manage_material.php">Manage Material</a>
        </div>
        <div class="logout">
    ...
    <div class="container">
        ...
        <div id="add">
            <a href="add_material.php" id="them">Add Material</a>
        </div>
        <?php
        ...
        // function getquantity($conn, $id)
        // {
        //     $query = "SELECT quantity FROM warehouses WHERE id = $id";
        ...
        function displayProducts($conn, $result)
        {
            if (mysqli_num_rows($result) > 0) {
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    // $getid = getID($conn, $row['id']);
                    // $quantity = getquantity($conn, $row['quantity']);
                    $material_name = getmaterial($conn, $row['warehouse_id']);
                    echo '<tr>
                            <td>' . $row['id'] . '</td>
                            <td>' . $material_name . '</td>
                            <td>' . $row['quantity']. '</td>
                            ...
                            <td>
                                <a href="edit_material.php?id=' . $row["id"] . '">Sửa</a>
                            </td>
                            <td>
                                <a onclick="return confirm(\'Bạn có muốn xóa hay không?\')" href="delete_material.php?id=' . $row["id"] . '">Xóa</a>
                            </td>
                            </tr>';
                }
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            if (isset($_POST['txtSubmit1'])) {
                ...
                $query = "SELECT * from material where id like '%$keyword1%'";
            } elseif (isset($_POST['txtSubmit2'])) {
                $keyword = isset($_POST['txtKeyword']) ? $_POST['txtKeyword'] : '';
                $query = "SELECT s.*, w.material AS name_material
                            FROM material s
